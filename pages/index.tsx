...
const CONCENTRIC_SEAT_COUNT_IN_SUBSEGMENT_PER_ROW = CONCENTRIC_SEAT_COUNT_IN_SEGMENT_PER_ROW.map((seatCount, rowIndex) => rowIndex > LAST_NON_SUBSEGMENTED_ROW_INDEX ? seatCount / 2 : seatCount)
const CONCENTRIC_SEAT_COUNT_PER_ROW = CONCENTRIC_SEAT_COUNT_IN_SEGMENT_PER_ROW.map((seatCountInSegment) => seatCountInSegment * SEGMENT_COUNT)
const PARALLEL_SEAT_SPACING = (Math.PI*(INITIAL_ROW_DISTANCE+2*REFERENCE_CONCENTRIC_SEAT_ROW_INDEX-SEAT_RADIUS)-SEAT_RADIUS*2*CONCENTRIC_SEAT_COUNT_PER_ROW[REFERENCE_CONCENTRIC_SEAT_ROW_INDEX])/(CONCENTRIC_SEAT_COUNT_PER_ROW[REFERENCE_CONCENTRIC_SEAT_ROW_INDEX]-1)
...
const CONCENTRIC_SEAT_COUNT_TOTAL = CONCENTRIC_SEAT_COUNT_PER_ROW.reduce((sumSoFar, rowSeatCount) => sumSoFar + rowSeatCount)
const CONCENTRIC_SEAT_INDEX_LOWER_BOUND_PER_ROW = CONCENTRIC_SEAT_COUNT_PER_ROW.reduce<number[]>((accumulator, rowSeatCount) => [...accumulator, (accumulator[accumulator.length-1] || -1) + rowSeatCount], [])
...
function translateSeatIndexToSeatNumber(seatIndex: number): number {
  ...
  if (seatIndex >= CONCENTRIC_SEAT_COUNT_TOTAL) {
    ...
    // Concentric rows
    baseOffset = 64
    const rowIndex = CONCENTRIC_SEAT_INDEX_LOWER_BOUND_PER_ROW.findIndex((seatIndexLowerBound) =>seatIndexLowerBound >=seatIndex)
    ...
    const isInSubsegment = rowIndex > LAST_NON_SUBSEGMENTED_ROW_INDEX && seatIndexWithinRow % seatCountInSegment >= Math.ceil(seatCountInSegment / 2)
    ...
    if (isInSubsegment) {
      seatNumberSegmentationOffset = CONCENTRIC_SEAT_COUNT_IN_SUBSEGMENT_PER_ROW.reduce((sumSoFar, seatCount) => sumSoFar + seatCount, 0) + CONCENTRIC_SEAT_COUNT_IN_SUBSEGMENT_PER_ROW.slice(LAST_NON_SUBSEGMENTED_ROW_INDEX+1,rowIndex).reduce((sumSoFar, seatCount) => sumSoFar + seatCount, 0)
    } else {
      seatNumberSegmentationOffset = CONCENTRIC_SEAT_COUNT_IN_SUBSEGMENT_PER_ROW.slice(0,rowIndex).reduce((sumSoFar, seatCount) => sumSoFar + seatCount, 0)
    }
    return baseOffset + seatNumberSegmentationOffset + CONCENTRIC_SEAT_COUNT_TOTAL / 4 * segmentIndex + seatIndexWithinSubsegment
  }
}
...
const Home: NextPage = () => {
    ...
    <div className={styles.container}>
      <main className={styles.main}>
        ...
        <Canvas camera={{position: [0,0,40]}}>
          ...
          <Seat key={i} position={[x, y, 0]} onHoverUpdate={(e) => {
  ...
  {hoverCoordinates && hoverIndex !== null && <div style={{position: 'absolute', left: 0, top: 0, transform: `translate(calc(${hoverCoordinates[0]}px - 100%), calc(${hoverCoordinates[1]}px - 100%))`, background: 'white', border: '3px solid black', padding: '0.5rem', pointerEvents: 'none'}}><b>Poseł nr {translateSeatIndexToSeatNumber(hoverIndex) || `#${hoverIndex}`}</b></div>}
  </>
}
